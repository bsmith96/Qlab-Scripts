...
def Fader():
	typ, ch = raw_input("Enter Channel Type and No. (e.g. 'ch 3'): ").split()
	while typ.isdigit() == True or ch.isdigit() == False:
		typ, ch = raw_input("ERROR: Enter Channel Type and No. (e.g. 'ch 3'): ").split()
	ch = int(ch)
	if typ == "ch":
		xtyp = format(51, '02X')
		while ch > 64:
...
def OnCue():
	...
	while typ.isdigit() == True or ch.isdigit() == False:
		typ, ch = raw_input("ERROR: Enter Channel Type and No. (e.g. 'ch 5'): ").split()
	ch = int(ch)
	op = raw_input("Enter Operation (on, off, cue, nocue): ")
	while op.isdigit() == True:
		op = raw_input("ERROR: Enter Operation (on, off, cue, nocue): ")
...
def SendToMix():
	ch = raw_input("Input Channel No: ")
	while ch.isdigit() == False:
		ch = raw_input("ERROR: Input Channel No: ")
	while int(ch) > 64:
		ch = input("Re-Enter Input Channel No: ")
		
	mix = raw_input("Send to Mix: ")
	while mix.isdigit() == False:
		mix = raw_input("ERROR: Send to Mix: ")
	mix = int(mix)
	while mix > 16:
...
def Custom():
	...
	cust = raw_input("Enter Custom Fader Channel No (33-36 St. In): ")
	while cust.isdigit() == False:
		cust = raw_input("ERROR: Enter Custom Fader Channel No (33-36 St. In): ")
	cust = int(cust)
	while cust > 68:
		print "error"
	...
	if cust > 32:
		...
		cust = format(cust-1, '02X')
		typ = raw_input("Enter Channel Type (e.g. 'ch / mix' , 'x' for NO Assign): ")
...
while main != 5:
	Menu()
	main = raw_input("Type in a number (1-6): ")
	if main == "q":
		exit()
	while main.isdigit() == False:
		main = raw_input("ERROR: Type in a number (1-6): ")
	main = int(main)
	...
	while main == 1:
		...
		time.sleep(0.5)
		print ""
		t = raw_input("Press 'enter' to Repeat, or 'q' to return to Menu: ")
		if t == "q":
			print ""
			main = 0
		else:
